...
function TypingIndicator() {
  return (
    <div className="flex gap-1.5 px-4 py-3 rounded-2xl w-fit animate-pulse card-glass">
      {[0, 150, 300].map(delay => (
        <div key={delay} className="w-2 h-2 rounded-full animate-bounce"
          style={{ animationDelay: `${delay}ms`, background: 'var(--blue)' }} />
      ))}
    </div>
  );
}
...
function MessageBubble({ message }) {
  ...
  if (isUser) {
    return (
      <div className="flex justify-end animate-fade-in">
        <div className="max-w-xs sm:max-w-md lg:max-w-xl px-4 py-3 rounded-2xl rounded-br-md text-white shadow-md" style={{ background: 'var(--blue)' }}>
  ...
  return (
    <div className="flex items-start gap-3 animate-fade-in">
      ...
      <div className="flex-1 card-glass rounded-2xl rounded-tl-md px-5 py-4">
        <div className="prose prose-sm max-w-none leading-relaxed space-y-2" style={{ color: 'var(--ink)' }}>
          <ReactMarkdown
            ...
            components={{
              ...
              code: ({children}) => <code style={{ background: 'var(--bg-subtle)', color: 'var(--blue)', padding: '0.1em 0.4em', borderRadius: 4, fontSize: '0.85em' }}>{children}</code>,
              pre: ({children}) => <pre style={{ background: 'var(--bg-subtle)', border: '1px solid var(--border)', borderRadius: 8, padding: '0.75rem 1rem', overflow: 'auto' }}>{children}</pre>,
      ...
      <div className="grid gap-2 w-full max-w-sm">
        {suggestions.map((text, i) => (
          <button key={i} onClick={() => onPromptClick(text)}
            className="px-4 py-3 rounded-xl text-sm text-left transition font-medium card-glass hover:border-violet-500 hover:shadow-md"
...
export default function App() {
  ...
  return (
    <div className="flex flex-col h-screen" style={{ background: 'var(--bg-page)' }}>
      {/* ── Top Bar ── */}
      <div className="flex items-center gap-3 px-4 py-3 shrink-0" style={{ background: 'var(--bg-card)', borderBottom: '1px solid var(--border)' }}>
        <button onClick={() => setSidebarOpen(o => !o)}
          className="p-1.5 rounded-lg transition lg:hidden" style={{ color: 'var(--ink-3)', background: 'var(--bg-subtle)' }}>
          <MessageSquare className="w-4 h-4" />
        </button>
        <Link to="/student" className="flex items-center gap-1.5 text-sm font-medium transition hover:underline" style={{ color: 'var(--blue)' }}>
          <ArrowLeft className="w-4 h-4" /> Dashboard
        </Link>

        {/* Mode toggle */}
        <div className="flex items-center gap-0.5 ml-3 rounded-lg p-0.5 border" style={{ background: 'var(--bg-subtle)', borderColor: 'var(--border)' }}>
          <button onClick={() => setChatMode("study")}
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-semibold transition`}
            style={chatMode === "study" ? { background: 'var(--bg-card)', color: 'var(--blue)', boxShadow: 'var(--shadow-sm)' } : { color: 'var(--ink-4)' }}>
            <BookOpen className="w-3.5 h-3.5" /> Study
          </button>
          <button onClick={() => setChatMode("support")}
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-semibold transition`}
            style={chatMode === "support" ? { background: 'var(--bg-card)', color: '#e11d48', boxShadow: 'var(--shadow-sm)' } : { color: 'var(--ink-4)' }}>
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* ── Sidebar ── */}
        <aside className={`fixed inset-y-0 left-0 z-40 w-64 flex flex-col pt-[53px] transform transition-transform lg:relative lg:translate-x-0 lg:pt-0 lg:z-auto ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}
            ...
            {sessionsLoading ? (
              ...
              chatSessions.map(session => (
                <div key={session.id}
                  ...
                  style={currentSessionId === session.id ? { background: 'var(--bg-hover)', color: 'var(--blue)' } : { color: 'var(--ink-3)' }}
                  onClick={() => loadSession(session.id)}>
                  <MessageSquare className="w-3.5 h-3.5 shrink-0" style={{ color: currentSessionId === session.id ? 'var(--blue)' : 'var(--ink-4)' }} />
                  <span className="text-xs truncate flex-1">
                    {session.title || "Untitled"}
                  </span>
                  <button onClick={e => deleteSession(e, session.id)}
                    className="opacity-0 group-hover:opacity-100 p-0.5 transition hover:text-red-500" style={{ color: 'var(--ink-4)' }}>
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          {/* ── Input Dock ── */}
          <div className="p-4 shrink-0" style={{ background: 'var(--bg-card)', borderTop: '1px solid var(--border)' }}>
            <div className="max-w-3xl mx-auto space-y-2">
              {fileMeta && (
                <div className="flex items-center justify-between px-3 py-2 rounded-xl" style={{ background: 'var(--bg-subtle)', border: '1px solid var(--border)' }}>
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => fileInputRef.current?.click()} disabled={fileUploading || isLoading}
                  className="p-2 rounded-xl transition disabled:opacity-50" style={{ color: 'var(--ink-3)', background: 'var(--bg-subtle)' }}>
                ...
                <button onClick={handleSend} disabled={isLoading || fileUploading}
                  className="btn-gradient p-2.5 flex items-center justify-center">
