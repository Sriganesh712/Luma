    ...
    <div className="min-h-screen  p-6">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center gap-3 mb-2">
          <button onClick={() => navigate('/teacher')} className=" hover: transition">
            <ArrowLeft className="w-5 h-5" />
          </button>
          <h1 className=" text-xl font-bold flex-1">All Materials</h1>
          <Link
        ...
        {filtered.length > 0 && (
          <div className="card-glass p-4 mb-6 flex items-center justify-between">
            <div>
              <p className=" font-semibold text-sm">Share learning materials</p>
              <p className=" text-xs mt-0.5">Upload PDFs, slides, videos, and links for your students.</p>
            </div>
        ...
        <div className="relative mb-6">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 card-glass rounded-xl   text-sm focus:outline-none focus:border-indigo-500 transition"
          />
        </div>
            ...
            {Object.entries(grouped).map(([key, { className, items }]) => (
              <div key={key} className="card-glass overflow-hidden">
                <div className="px-6 py-3 border-b border-slate-100 flex items-center justify-between">
                  <h2 className=" font-semibold text-sm">{className}</h2>
                  <span className=" text-xs">{items.length} file{items.length !== 1 ? 's' : ''}</span>
                </div>
                <div className="divide-y divide-slate-100">
                  {items.map(m => {
                    ...
                    return (
                      <div key={m.id} className="flex items-center gap-4 px-6 py-4 hover: transition group">
                        <div className="w-9 h-9  group-hover: rounded-lg flex items-center justify-center shrink-0 transition">
                          <Icon className="w-4 h-4 text-indigo-400" />
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className=" text-sm font-medium truncate">{m.title}</div>
                          <div className=" text-xs mt-0.5 uppercase">{m.type} · {new Date(m.created_at).toLocaleDateString()}</div>
                        </div>
                        <div className="flex items-center gap-2 shrink-0">
                          ...
                          <button
                            ...
                            className="p-1.5  hover:text-red-400 transition disabled:opacity-50"
                          >
