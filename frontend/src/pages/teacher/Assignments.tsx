    ...
    <div className="min-h-screen  p-6">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className=" text-xl font-bold flex-1">All Assignments</h1>
          <Link
          ...
          <div className="flex items-center gap-1 card-glass rounded-xl p-1">
            ...
            {(['all', 'draft', 'published', 'closed'] as StatusFilter[]).map(s => (
              <button key={s} onClick={() => setStatusFilter(s)}
                className={`px-3 py-1.5 rounded-lg text-xs font-medium capitalize transition ${statusFilter === s ? 'bg-indigo-600 text-white' : ' hover:'}`}>
            ...
            <div className="divide-y divide-slate-100">
              {filtered.map(a => (
                <Link key={a.id} to={`/teacher/assignments/${a.id}`}
                  className="flex items-center justify-between px-6 py-4 hover: transition group">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <ChevronRight className="w-4 h-4  group-hover: transition" />
                  </div>
                </Link>
              ))}
            </div>
